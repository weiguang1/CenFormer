...
def init_stratified_dataloader(cfg: DictConfig,
    ...
    # split_random_state controls BOTH partition stages (train/rest and
    # val/test); default 42 reproduces the released split exactly.
    split_rs = cfg.dataset.get('split_random_state', 42)
    split = StratifiedShuffleSplit(
        n_splits=1, test_size=val_length+test_length, train_size=train_length, random_state=split_rs)
    ...
    # fixed_split=True freezes the val/test partition across seeds (the
    # train/rest split above is already fixed at split_random_state), so seed
    # variance isolates pure training stochasticity. Default None keeps the
    # released behavior: the partition is drawn from the seeded global RNG.
    split2 = StratifiedShuffleSplit(
        n_splits=1, test_size=test_length,
        random_state=split_rs if cfg.dataset.get('fixed_split', False) else None)
