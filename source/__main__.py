...
def main(cfg: DictConfig):
    ...
    # Preserve the original seed -> W&B -> data -> model initialization order.
    os.environ.setdefault("WANDB_MODE", "offline")
    pooling = cfg.model.get('pooling', ['-', '-'])
    group_name = (f"CenFormer-{cfg.dataset.batch_size}"
                  f"_{cfg.model.get('nhead', '-')}"
                  f"_{cfg.model.get('num_MHSA', '-')}"
                  f"_{pooling[1]}")
    count = 0
    acc_list = []
    auc_list = []
    ...
    seeds = list(range(seed_offset, seed_offset + cfg.repeat_time))
