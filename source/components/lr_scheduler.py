...
class LRScheduler:
    ...
    def update(self, optimizer: torch.optim.Optimizer, step: int):
        ...
        for param_group in optimizer.param_groups:
            param_group['lr'] = self.lr * param_group.get('lr_mult', 1.0)


def lr_scheduler_factory(lr_configs: DictConfig, cfg: DictConfig) -> List[LRScheduler]:
    #return [LRScheduler(cfg=cfg, optimizer_cfg=lr_config) for lr_config in lr_configs]
    return [LRScheduler(cfg=cfg, optimizer_cfg=lr_configs)]
